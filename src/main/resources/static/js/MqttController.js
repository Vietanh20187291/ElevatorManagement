...
function MQTTOnMessageArrived(message) {
    ...
    for (var i = 0; i < slugs.length; i++) {
        if (topic == slugs[i] + "/elvtopc/indicator") {
            ...
            GUIShowIndicator(info, eleNames[i]);
        }
        ...
        if (topic == slugs[i] + "/elvtopc/callcar") {
            var payloadBytes = message.payloadBytes;
            var payloadHex = bytesToHex(payloadBytes);
...
function MQTTOpenDoor(eleno){
    var msg = new Uint8Array([0x01]);
    var topic = slugs[eleno]+"/pctoelv/status";
...
function MQTTcarCall(input, eleno) {
    var topic = slugs[eleno] + "/pctoelv/callcar";
...
function MQTTcallUp(input, eleno) {
    var topic = slugs[eleno] + "/pctoelv/callhallup";
    handleCallClick(input,topic, mqtt);
}


function MQTTcallDn(input, eleno) {

    var topic = slugs[eleno]+"/pctoelv/callhalldown";
    handleCallClick(input,topic);
}
